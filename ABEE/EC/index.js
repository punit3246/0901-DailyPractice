...
app.get("/delCart/:id",(req,res)=>{
  let q="delete from cart where id ='"+req.params.id+"'";
  let product;
...
app.get("/addCart/:id",(req,res)=>{
  let q="select name,sale_price,image from products where id='"+req.params.id+"'";
  let product;
  conn.query(q,(err,result)=>{
      if(err)throw err;
      else{
        product=result[0];
        console.log(product);
        q="insert into cart (username,name,sale_price,image,p_id) values ('"+req.session.username+"','"+product.name+"','"+product.sale_price+"','"+product.image+"','"+req.params.id+"')";
        conn.query(q,(err,result)=>{
          if(err)throw err;
          else{
            console.log(result);
...
app.get("/cart",(req,res)=>{
  let q="select * from cart where username='"+req.session.username+"'";
