...
app.get("/update/it/:id",function(req,res){
let sqlQuery = "SELECT * FROM items WHERE id=" + req.params.id+ " LIMIT 1";
let query = conn.query(sqlQuery, (err, results) => {
  if(err) throw err;
...
app.post('/updatedit',(req, res) => {
  let sqlQuery = "UPDATE items SET title='"+req.body.title+"', body='"+req.body.body+"' WHERE id="+req.body.id;
...
app.post("/signup",(req,res)=>{
  let data={email:req.body.email,username:req.body.name,passwd:req.body.password}
  let SqlQuery = "insert into userauth set ?"
